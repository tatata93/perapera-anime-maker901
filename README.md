...
- セル・レイヤー構造(Cut→セル→レイヤー→フレーム)。レイヤーパネルで追加/削除/移動/可視切替、
  種別設定(通常/色トレス線/作監修正)
- カラーパレット(プロジェクトに保存)
- **タイムシート(Xsheet)**: 原画工程、動画工程、シート確認を切替。原画を先に作り、
  その後で中割とCELL指定を行う。尺変更、1/2/3コマ打ち、コピー、空セル、直接編集に対応
...
2. `原画工程`で次の要所のコマを選び、`原画を作る`を押して描く
3. 原画が揃ったら`動画工程`へ切り替え、原画の間で`中割を作る`を押して描く
4. CELL列で1/2/3コマ打ちや「同じ絵を延長」を使い、最終的な表示タイミングを整える

黄色い「A 原画」は原画番号や中割指示、青い「A セル」は再生・書き出しで表示する
原画または中割の番号を表す。
