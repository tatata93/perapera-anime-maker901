...
TEST_CASE("makeCylinderMeshData: 妥当なジオメトリを生成する", "[previz]") {
...
TEST_CASE("makeSphereMeshData: 妥当なジオメトリを生成する", "[previz]") {
...
TEST_CASE("loadStlMesh: バイナリSTLを読み込み、自動フィットする", "[previz][stl]") {
...
TEST_CASE("loadStlMesh: ASCII STLを読み込む", "[previz][stl]") {
...
TEST_CASE("loadStlMesh: 異常系(存在しないファイル/空ファイル)はエラーを返す", "[previz][stl]") {
    previz::MeshData data;
    std::string error;
